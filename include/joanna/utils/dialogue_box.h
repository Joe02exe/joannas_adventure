...
class DialogueBox {
  public:
    ...
    void setDialogue(
        const std::vector<std::string>& messages, const void* owner = nullptr
    );
    ...
    bool isTyping() const {
        return visibleCharCount < currentDialogue.length();
    }
    ...
    void update(float dt, const jo::Vector2f& targetPosition);
  ...
  private:
    ...
    jo::RectangleShape bubbleBackground;
    jo::ConvexShape bubblePointer;
    jo::Vector2f textPosition;
    const void* owner = nullptr;
    static constexpr float BUBBLE_WIDTH = 500.0f;
