...
TEST_F(InventoryTest, CapacityLimitReached) {
    Inventory inv(2);
    Item itemA = createItem("A", false);
    Item itemB = createItem("B", false);
    Item itemC = createItem("C", false);
...
TEST_F(InventoryTest, RemoveItem) {
    Inventory inv(10);
    Item potion = createItem("heal", true);

    inv.addItem(potion, 10);

    uint32_t removed = inv.removeItem("heal", 4);
    EXPECT_EQ(removed, 4);
    EXPECT_EQ(inv.getQuantity("heal"), 6);

    removed = inv.removeItem("heal", 10);
    EXPECT_EQ(removed, 6);
    EXPECT_EQ(inv.getQuantity("heal"), 0);
    EXPECT_EQ(inv.slotsUsed(), 0);
}
...
TEST_F(InventoryTest, SelectionNavigation) {
    ...
    EXPECT_EQ(inv.getSelectedItemId(), "B");

    inv.selectNext();
    EXPECT_EQ(inv.getSelectedSlotIndex(), 2);
    EXPECT_EQ(inv.getSelectedItemId(), "C");
